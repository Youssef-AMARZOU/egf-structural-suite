...
fn v_rdc_calc(fck: f64, gc: f64, rho_l: f64, d: f64, bw: f64) -> f64 {
    let crdc = 0.18 / gc;
...
fn sigma_cd_calc(n_ed: f64, bw: f64, d: f64) -> f64 {
    let ac = bw * d;
    n_ed * 1000.0 / ac
}
...
fn stirrup_spacing_calc(
    v_ed: f64, v_rdc: f64, v_rdc_max: f64,
    bw: f64, d: f64, fyk: f64, gs: f64,
) -> (f64, f64) {
    if v_ed <= v_rdc {
        let fctd = fyk / gs * 0.1;
        let a_sw = bw * fctd / 1000.0;
        (d, a_sw)
    } else {
        let fywd = fyk / gs;
        let z = 0.9 * d;
        let v_rd = (v_ed - v_rdc) * 1000.0;
        let a_sw = v_rd / (z * fywd);
        let spacing = (a_sw * z * fywd / v_rd * 1000.0).min(d);
        (spacing, a_sw)
    }
}
...
pub fn calculate_cisai_rect_139(
    ...
    let v_rdc = v_rdc_calc(p.fck, p.gc, p.rho_l, d, bw);
    let v_rdc_max = v_rdc_max_calc(p.fck, p.gc, bw, d);
    let sigma_cd = sigma_cd_calc(p.n_ed, bw, d);
    ...
    let (spacing, a_sw) = stirrup_spacing_calc(
        v_ed_max, v_rdc, v_rdc_max, bw, d, p.fyk, p.gs,
    );
    ...
    if ratio_v_max > 1.0 {
        diag.push(format!(
            "VEd,Max={:.1}kN > VRd,max={:.1}kN — augmenter bw ou d",
            v_ed_max, v_rdc_max
        ));
    }
    ...
    let verdict = if ratio_v_max > 1.0 {
        ...
        format!(
            "ATTENTION: VEd={:.1}kN > VRd,c={:.1}kN — α_sw={:.1}mm², espacement={:.0}mm",
            v_ed_max, v_rdc, a_sw, spacing
        )
    } else {
        format!(
