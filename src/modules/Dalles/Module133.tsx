...
export default function Module133() {
  ...
  const { data: res, error: err, live } = useModuleCalc<VerificationDallesPoinconnementInputs, VerificationDallesPoinconnementOutput>(
    'calculate_verification_dalles_poinconnement_133', inp,
  );
  ...
  return (
    <Workstation
      title="133 Vérif. Poinçonnement"
      subtitle="Vérification poinçonnement — ouvertures, η, périmètres"
      status={status}
      live={live}
      params={
        <>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          {slider('d_x', 'dx', 'mm', 80, 400, 5)}
          {slider('d_y', 'dy', 'mm', 80, 400, 5)}
          {slider('b_vd', 'Bvd', 'kN', 0, 5000, 10)}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {slider('asx', 'Asx', 'cm²', 0, 100, 0.5)}
          {slider('asy', 'Asy', 'cm²', 0, 100, 0.5)}
        </div>
        ...
        {inp.has_opening && (
          <div className="grid grid-cols-2 gap-2">
            {slider('opening_l1', 'l1', 'mm', 0, 2000, 50)}
            {slider('opening_l2', 'l2', 'mm', 0, 2000, 50)}
            {slider('opening_x', 'x₀', 'mm', -3000, 3000, 50)}
            {slider('opening_y', 'y₀', 'mm', -3000, 3000, 50)}
          </div>
        )}
        ...
        {inp.has_shear_reinf && slider('asw', 'Asw', 'cm²/m', 0, 5, 0.05)}
        ...
        <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0f172a] p-4">
          ...
          {res && (
            <div className="font-mono text-xs space-y-1">
              ...
              <FormulaCard
                title="Vérification poinçonnement"
                latex={String.raw`v_{Ed} = \frac{\beta V_{Ed}}{u_1 d} \le v_{Rd,c}`}
                description="Ouvertures, bords, angles"
                status={status === 'computing' ? 'neutral' : status}
                variables={[
                    { symbol: String.raw`\eta`, meaning: 'Facteur de bord', value: res.d_mean.toFixed(1) },
                    { symbol: String.raw`u_1`, meaning: 'Périmètre réduit', value: res.u1.toFixed(0) },
                ]}
              />
