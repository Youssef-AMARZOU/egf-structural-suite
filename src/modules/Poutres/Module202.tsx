              ...
              {(() => {
                ...
                return (
                  <>
                    {isT ? (
                      <>
                        <rect x={cx - bPx / 2} y={yTop} width={bPx} height={hfPx} fill="#E2E8F0" stroke="#333" />
                        <rect x={cx - bwPx / 2} y={yTop + hfPx} width={bwPx} height={hPx - hfPx} fill="#E2E8F0" stroke="#333" />
                      </>
                    ) : (
                      <rect x={cx - bPx / 2} y={yTop} width={bPx} height={hPx} fill="#E2E8F0" stroke="#333" />
                    )}
                    ...
                    <text x={cx} y={yTop + hPx + 16} fontSize={10} fill="#333" textAnchor="middle">x={result.x.toFixed(0)} mm — pivot {result.pivot}</text>
                  </>
                );
              })()}
            </>);
      ...
      results={<>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { label: 'As', value: res.as_req.toFixed(0), unit: 'mm²', color: 'bg-blue-50' },
              { label: "As'", value: res.as_comp.toFixed(0), unit: 'mm²', color: 'bg-purple-50' },
              { label: 'x', value: res.x.toFixed(0), unit: 'mm', color: 'bg-orange-50' },
              { label: 'μ', value: res.mu.toFixed(3), unit: '', color: 'bg-gray-50' },
            ].map((item, i) => (
